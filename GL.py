...
class Renderer(object):
    ...
    def glCreateRotationMatrix(self, rotate=V3(0,0,0)):
        pitch = Mlib.Deg2rad(rotate.x)
        yaw = Mlib.Deg2rad(rotate.y)
        roll = Mlib.Deg2rad(rotate.z)
    ...
    def glLookAt(self, eye, camPosition = V3(0,0,0)):
        forward = Mlib.sub(camPosition, eye)
        forward = forward / np.linalg.norm(forward)

        right = Mlib.cross(V3(0,1,0), forward)
        right = right / np.linalg.norm(right)

        up = Mlib.cross(forward, right)
